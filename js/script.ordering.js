document.addEventListener('DOMContentLoaded', () => {
  ...
  const removeActiveClassFromAddressInput = () => {
    addressSelect.classList.contains('address_show') ? addressSelect.classList.remove('address_show') : null;
    addressInput.classList.contains('address_show') ? addressInput.classList.remove('address_show') : null;
  };
  ...
  const removeActiveClassToorderCount = i => {
    if (Number.isInteger(i)) {
      ...
      orderCount.forEach(elem => {
        elem.classList.contains('order__count_active') ? elem.classList.remove('order__count_active') : null;
      });
    }
  };
  ...
  const calculateTotalPrice = () => {
    ...
    allPrice.innerHTML = newMass.reduce((accumulator, currentValue) => accumulator + currentValue, initialValue);
  };
